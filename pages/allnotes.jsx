...
const Allnotes = () => {
	...
	const searchFilter = (text) => {
		...
		let filtered = notesCopy.filter(
			(item) =>
				item.title.toString().toLowerCase().includes(searchText) ||
				(Array.isArray(item.content) &&
					getContentChecklist(item.content)
						.toLowerCase()
						.includes(searchText)) ||
				(!Array.isArray(item.content) &&
					...
					<div className={style.dropdown}>
						...
						<ul className="dropdown-menu">
							<li>
								<a
									className={`dropdown-item ${
										sortlistActive == "color" ? "active" : ""
									}`}
									onClick={() => {
										SetSortlistActive("color");
									}}>
									By Color
								</a>
							</li>
							<li>
								<a
									className={`dropdown-item ${
										sortlistActive == "date_modified" ? "active" : ""
									}`}
									onClick={() => {
										SetSortlistActive("date_modified");
									}}>
									By Modified Time
								</a>
							</li>
							<li>
								<a
									className={`dropdown-item  ${
										sortlistActive == "date_created" ? "active" : ""
									}`}
									onClick={() => SetSortlistActive("date_created")}>
									By Created Time
								</a>
							</li>
							<li>
								<a
									className={`dropdown-item  ${
										sortlistActive == "title" ? "active" : ""
									}`}
									onClick={() => SetSortlistActive("title")}>
									Alphabetically
								</a>
							</li>
						</ul>
				...
				<section className="dropdown">
					...
					<ul className={`${style.add_new} dropdown-menu`}>
						...
						<Link passHref href="/checklist">
							<li className="dropdown-item">Checklist</li>
						</Link>
					</ul>
				</section>
			</header>
									...
									<Link href={`/todo/${data.id}`}>
										<div
											...
											<div
												...
												<p>{data.title}</p>
												<Image
													src={"/note-assets/check-icon.png"}
													width={20}
													height={20}></Image>
											</div>

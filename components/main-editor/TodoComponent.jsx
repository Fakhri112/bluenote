...
const TodoComponent = (props) => {
	...
	const handleSave = async () => {
		if (!saveTodo || (inputTitle.length == 0 && todoData.length == 0))
	...
	const handleUpdate = async () => {
		...
		try {
			const send = await axios.put(
				`/api/sendnote?type=${collectionName}&id=${todoID}`,
				payload,
			);
			if (send.data.status == 200) {
				setSavePopUp({ ...savePopUp, saved: true, saving: false });
	...
	useEffect(() => {
		inputTitle.length == 0 && todoData.length == 0
			...
			: SetEmptyNote(false);
		if (archiveConfirm) {
			handleArchiveTrash("archives", "todos");
		}
		if (trashConfirm) {
			handleArchiveTrash("trashes", "todos");
		}
		if (restore.unarchive) {
			handleArchiveTrash("todos", "archives");
		}
		if (restore.untrash) {
			handleArchiveTrash("todos", "trashes");
		}
		if (deletePerm) {
		...
		<div>
			...
			<main className={style.note_panel}>
				<LeftButton
					...
					backsave={backsave}
					checkicon={(val) => {
						SetEditMode(val), SetBacksave(false);
					}}
				...
				<section className={`${style.notepad} ${notepadColor.new_color}`}>
					...
					{savePopUp.success && !props.isArchive ? (
						<NoteSaved status={"Success"} />
					) : null}
					<div className={style.note_title}>
						<input
							type="text"
							autoFocus={Object.keys(props).length === 0 ? true : false}
							className={strikeTitle()}
					...
					{editMode && !props.isTrash ? (
						...
						<Todo
							...
							updateData={(data) => SetTodoData(data)}
							tgrTodoData={(prop) => SetTgrTodoData(prop)}
							editMode={(value) => (props.isTrash ? true : SetEditMode(value))}
							indexData={(index) => SetIndexFocus(index)}
						/>
					)}
				</section>
				<RightButton
